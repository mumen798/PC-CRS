...
class UNICRS():
    ...
    def get_conv(self, conv_dict):
        ...
        recommender_text = gen_str[gen_str.rfind('System:') + len('System:') + 1:]
        recommender_text = recommender_text.replace('<|endoftext|>', '')

        meta_info = {}
        rec_info = self.id2info[self.entityid2id[self.item_to_recommend[0][0]]]
        print(rec_info)
        ...
        if '<movie>' in recommender_text or '<mask>' in recommender_text:
            if rec_item != self.last_rec:
